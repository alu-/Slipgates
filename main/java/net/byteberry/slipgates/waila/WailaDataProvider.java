package net.byteberry.slipgates.waila;
...
public class WailaDataProvider implements IWailaDataProvider {
	// TODO @see
	// https://github.com/pahimar/Equivalent-Exchange-3/blob/bce498695e1454cc48c4263be9c0bb1ee063dbc5/src/main/java/com/pahimar/ee3/waila/WailaDataProvider.java
	...
	public static void callbackRegister(IWailaRegistrar register) {
		 ...
		 * block.
		 */
		register.registerBodyProvider(instance, PortalCapacitor.class);
		register.registerBodyProvider(instance, PortalCharger.class);
	...
	public List<String> getWailaBody(ItemStack itemStack, List<String> currentTip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
		/**
		 * In all data provider methods we are given access to several things.
		 * The first is itemStack which is an ItemStack representation of the
		 * block being looked at. The second is currentTip which is a list of
		 * string, each string in this list represents a line in the body of the
		 * tooltip, this can be used to add (and remove) information to the
		 * tooltip. The third is accessor which is a wrapper object in Waila
		 * which holds all relevant information. The final thing we are given is
		 * access to the configuration. (note: this method is only registered to
		 * work with blocks which extend BlockOre BlockContainer and
		 * BlockWailaTestBlock. It will only be called when looking at those
		 * blocks.)
		 */
		
		/**
		 * The first thing we are going to add is the amount of exp dropped by a
		 * block. Since any block can have this value, we only need to check if
		 * this feature is enabled in the configuration. For this tutorial we
		 * are going to assume the player isn't using the fortune effect,
		 * although the level of fortune can be obtained by grabbing the players
		 * held item. It's also worth noting that getExpDrop() is not meant to
		 * be called for grabbing information. Although some blocks like the
		 * test block provide a constant values, blocks like coal ore will have
		 * this value change based on random factors.
		 */
...
//		}
		...
		/**
		 * Finally we are going to print the tileid for all tile entities. For
		 * this we are going to check the config, and if the TileEntity is not
		 * null.
		 *
		 */
...
//		}
		...
		if (accessor.getTileEntity() instanceof TileEntityPortalCapacitor) {
			TileEntityPortalCapacitor capacitor = (TileEntityPortalCapacitor) accessor.getTileEntity();
			currentTip.add("Power: " + capacitor.getEnergyStored(ForgeDirection.DOWN) + " / " + capacitor.getMaxEnergyStored(ForgeDirection.DOWN) + " RF");
		}
		
		if (accessor.getTileEntity() instanceof TileEntityPortalCharger) {
			TileEntityPortalCharger charger = (TileEntityPortalCharger) accessor.getTileEntity();
			currentTip.add("Power: " + charger.getEnergyStored(ForgeDirection.DOWN) + " / " + charger.getMaxEnergyStored(ForgeDirection.DOWN) + " RF");
		}
